...
console.log('To start a session, please start by entering: ' + chalk.yellow('start'));
...
async function main() {
  ...
  if (getSessionId.success === true) {
    ...
    if(sendingChatRequest === true){
        ...
        var pullmessageorg = await helperFunctions.pullingMessages(affinity, sessionkey);
        ...
        while(pullmessageorg.messages[0].type != 'ChatEnded') {
            ...
            if(pullmessageorg.messages[0].type === 'ChatEstablished'){
                ...
                console.log('\n' + chalk.cyanBright(pullmessageorg.messages[0].message.name) + chalk.white(' is here to help you. Should be joining you any second now.'));
            }
            if(pullmessageorg.messages[0].type === 'ChatMessage'){
                console.log('\n' + chalk.yellowBright(pullmessageorg.messages[0].message.name) + ' : ' + pullmessageorg.messages[0].message.text +  '\n');

                let text = inputReader.readLine(chalk.red("You : "));

                const sendMessage = await helperFunctions.sendMessages(text, affinity, sessionkey);
            ...
            const pullingMessagesAgain = await helperFunctions.pullingMessages(affinity, sessionkey);
